...
class Markov_Model(persistent.Persistent):
    ...
    def compute_probability(self, state):
        ...
        # The final probability should be the prob of starting in the first letter, plus the transitions
...
######################
######################
class Group_of_Markov_Models_n1(Module, persistent.Persistent):
    cmd = 'markov_models_n1'
    description = 'This module uses a smarter way of separating the states of the tuple and implements markov chains of first order over those states in a LABEL. ' + yellow('Warning') + ', if the original models or labels are deleted, you should fix these models by hand.'
    ...
    def __init__(self):
        ...
        super(Group_of_Markov_Models_n1, self).__init__()
    ...
    def printstate(self, markov_model_id):
        ...
        print_info('Transition Matrix: {}'.format(markov_model.get_matrix()))
...
####
        ...
        for test_model in self.get_markov_models():
            ...
            if ( (filter != "" and self.apply_filter(test_model)) or (test_models_ids != "" and test_model_id in test_models_ids)) and test_model_id != train_model_id and train_protocol == test_protocol:
                ...
                for threshold in np.arange(1.1,2.1,0.1):
                    ...
                    # Not sure if I should store the self prob
                    #original_self_prob = train_model.get_self_probability()
                    # For each test state
                    index = 0
                    ...
                    if exit_threshold_for:
                        ...
                        break
                # After the for of the thresholds, restore the original matrix and prob
                train_model.get_matrix(original_matrix)
    ...
    def run(self):
        ...
        if not __database__.has_structure(Group_of_Markov_Models_n1().get_name()):
            print_info('The structure is not registered.')
            __database__.set_new_structure(Group_of_Markov_Models_n1())
        else:
            main_dict = __database__.get_new_structure(Group_of_Markov_Models_n1())
        ...
        # Run
        super(Group_of_Markov_Models_n1, self).run()
...
__group_of_markov_models__ = Group_of_Markov_Models_n1()
