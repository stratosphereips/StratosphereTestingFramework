...
# This module implements markov chains of second order over the letters in the chain of states of the behavioral models.
import persistent
import pykov
...
class Markov_Model(persistent.Persistent):
    ...
    def set_vectorstate(self, vectorstate):
        self.vectorstate = vectorstate

    def get_vectorstate(self):
    ...
    def compute_probability(self, state_string):
        ...
        probability = 0
        ignored = 0
        # Before continuing we should convert the state in a two-letter vector. Because before the state acted as a one letter vector.
        state = []
        j = 0 
        while j < len(state_string):
            state.append(state_string[j:j+2])
            j+=1
        i = 0
        ...
        while i < len(state) and len(state) > 2:
            try:
                ...
                if temp_prob != float('-inf'):                
                    ...
                    # Here is our trick. If two states are not in the matrix... ignore the transition.
                    #if '#' not in vector:
                        #ignored += 1
                    continue
            except IndexError:             
...
class Group_of_Markov_Models_2(Module, persistent.Persistent):
    ...
    def list_markov_models(self, filter):
        ...
        print(table(header=['Id', 'State Len', '# Connections', 'Label', 'Needs Regenerate', 'First 100 states in State'], rows=rows))
    ...
    def printstate(self, markov_model_id):
        ...
        state = markov_model.get_vectorstate()
        #len_state = len(state) if not len(state)%2 else len(state)-1
